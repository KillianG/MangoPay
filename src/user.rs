...
impl Mangopay {
    ...
    pub async fn create_user(self: &Mangopay, user_infos: &CreateUserBody) -> Result<User, reqwest::Error> {
        let user_response = match self.create_post_api_call("users/natural/".parse().unwrap()).json(user_infos).send().await {
            Ok(resp) => resp,
            Err(e) => return Err(e)
        };
